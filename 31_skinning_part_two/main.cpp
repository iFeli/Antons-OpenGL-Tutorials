...
animation */
void skeleton_animate( Skeleton_Node* node, mat4 parent_mat, mat4* bone_offset_mats, mat4* bone_animation_mats );
...
void skeleton_animate( Skeleton_Node* node, mat4 parent_mat, mat4* bone_offset_mats, mat4* bone_animation_mats ) {
  ...
  for ( int i = 0; i < node->num_children; i++ ) { skeleton_animate( node->children[i], our_mat, bone_offset_mats, bone_animation_mats ); }
}
...
bool load_mesh( const char* file_name, GLuint* vao, int* point_count, mat4* bone_offset_mats, int* bone_count, Skeleton_Node** root_node ) {
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
  ...
  while ( !glfwWindowShouldClose( g_window ) ) {
    ...
    if ( monkey_moved ) {
      skeleton_animate( monkey_root_node, identity_mat4(), monkey_bone_offset_matrices, monkey_bone_animation_mats );
      glUseProgram( shader_programme );
